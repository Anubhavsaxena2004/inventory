...
def test_frontend_backend_integration():
    ...
    try:
        ...
        if response.status_code == 201:
            print("  PASS Customer added successfully from frontend")
            ...
            if list_response.status_code == 200:
                ...
                if found_customer:
                    print("  PASS Customer found in customer list")
                    print("  PASS Frontend-Backend integration working correctly")
                    return True
                else:
                    print("  FAIL Customer not found in customer list")
                    return False
            else:
                print("  FAIL Failed to retrieve customer list")
                return False
        else:
            print(f"  FAIL Failed to add customer (Status: {response.status_code})")
            return False
...
def test_frontend_components():
    ...
    print("\nTesting Frontend Component Data Loading...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("  PASS Frontend can load dashboard data")
        else:
            print("  FAIL Dashboard data not available")
            return False
    except Exception as e:
        print(f"  FAIL Dashboard data test failed: {e}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"  PASS Customer data available: {len(customers)} customers")
            print("  PASS Frontend can load customer data")
        else:
            print("  FAIL Customer data not available")
            return False
    except Exception as e:
        print(f"  FAIL Customer data test failed: {e}")
        return False
...
def main():
    ...
    if passed == total:
        ...
        print("PASS All connectivity tests passed!")
        print("PASS Frontend (localhost:5173) can successfully communicate with Backend (localhost:8000)")
        print("PASS The system is ready for full-stack development!")
    else:
        print(f"\nWARNING {total-passed} connectivity tests failed.")
