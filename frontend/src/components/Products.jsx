...
export default function Products(){
  ...
  useEffect(()=>{
    fetchWithAuth('/api/settings/products/').then(r=>r.json()).then(d=>setProducts(d.products||[])).catch(e=>setMsg(String(e)))
  },[])
    ...
    <div className="card">
            ...
            {filtered.map(p=> (
              <tr key={p.id}>
                ...
                <td>
                  ...
                  <button className="btn small" onClick={async ()=>{ await fetchWithAuth('/api/settings/products/',{ method:'DELETE', headers:{'Content-Type':'application/json','X-Admin':'true'}, body: JSON.stringify({id:p.id}) }); const r = await fetchWithAuth('/api/settings/products/'); const d = await r.json(); setProducts(d.products||[]) }}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {confirming && (
        <Modal title={form.id? 'Confirm Update Product':'Confirm Add Product'} onClose={()=>setConfirming(false)} onConfirm={async ()=>{
          ...
          const r = await fetchWithAuth('/api/settings/products/',{ method, headers:{'Content-Type':'application/json','X-Admin':'true'}, body: JSON.stringify({ id: form.id||undefined, name: form.name, category: form.category, unit_price: Number(form.unit_price||0), quantity: Number(form.quantity||0), min_quantity: Number(form.min_quantity||0) }) })
          const d = await r.json()
          if(r.ok){ const re = await fetchWithAuth('/api/settings/products/'); const dx = await re.json(); setProducts(dx.products||[]); setForm({id:null,name:'',category:'',unit_price:'',quantity:'',min_quantity:''}); setErrors({}); setMsg('') }
